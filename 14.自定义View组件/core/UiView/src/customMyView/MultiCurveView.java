 ...
 * */
public class MultiCurveView extends ViewGroup{
	...
	public MultiCurveView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
	...
	public MultiCurveView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//			fillendColor = arr.getColor(R.styleable.CurveView_fillendColor, fillendColor);
	...
	private void init(Context context){
        ...
        value_lst_lst = new ArrayList<List<String>>();
        for(int i=0;i<5;i++){
        	int n = i;
        	int m = 3;
        	if(n==2) m = 5;
            ...
            value_lst.add("0-"+String.valueOf(21+n*11));
            value_lst.add("1-"+String.valueOf(61-n*10+m));
            value_lst.add("2-"+String.valueOf(32+n*11));
            value_lst.add("4-"+String.valueOf(52+n*5+m));    
            value_lst.add("7-"+String.valueOf(59+n*6)); 
            value_lst.add("8-"+String.valueOf(49+n*8)); 
            value_lst.add("9-"+String.valueOf(89-n*10+m)); 
            value_lst.add("13-"+String.valueOf(49+n*3)); 
            value_lst.add("18-"+String.valueOf(38+n*7+m)); 
            value_lst.add("21-"+String.valueOf(49+n*10));
            value_lst_lst.add(value_lst);
        }
...
//	    private  boolean  IsFillShadow = true;   //默认  控件   是否填充阴影
	    ...
	    private  int[]  viewColors = {0xFFF84C4C, 0xFFB666C7,0xFF398D17,0xFF33DF8B,
	    		                     0xFFF6D532,0xFFFeA934,0xFFF933AA,0xFF398DE7};   //默认  控件 曲线  颜色数组
	    
	    //注意 该数值链表 一定要按x量从小到大排序  否则会曲线错乱
	    private  List<String> value_lst = null;  //字符格式     10-27 数组   x-y 以圆点为0
	    private  List<List<String>> value_lst_lst = null;  //字符格式     List<10-27 数组> 代表多条链表 最多限8条
		...
		protected void dispatchDraw(Canvas canvas) {
	        ...
	        if(value_lst_lst ==null || value_lst_lst.size()==0) return;  
	        int num = value_lst_lst.size()>8?8:value_lst_lst.size(); //限定最多只显示  8条曲线
	        for(int j=0;j<num;j++){
	        	if(value_lst != null) value_lst.clear();
	        	value_lst = value_lst_lst.get(j);
	        	if(value_lst ==null || value_lst.size()==0) continue; 
...
//	        Log.i("Tag>>BarGraphView->onMeasure",String.valueOf(widthSize)+"   "+String.valueOf(heightSize));
	    ...
	    //设置 柱状图 数据
		public void setValue_lst(List<List<String>> lst_lst){
			if(lst_lst == null) return;
			if(value_lst_lst != null){
				for(int i=0;i<value_lst_lst.size();i++){
					value_lst_lst.get(i).clear();
				}
				value_lst_lst.clear();
			}
			value_lst_lst = lst_lst;
		}
		//设置 曲线颜色
