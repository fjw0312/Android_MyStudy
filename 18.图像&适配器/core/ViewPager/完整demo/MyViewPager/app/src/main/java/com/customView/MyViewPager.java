...
public class MyViewPager extends ViewPager{
    ...
    private void init_view(Context context){
     ...
     //   AdapterPagerFragment adapter = new AdapterPagerFragment( mContext.getSupportFragmentManager() ); //需要在FragmentActivity类里
     //   this.setAdapter(adapter);
    }
    //更新 控件页面数据
    public void Update(List<Fragment> mFragments){
        if(adapter!=null){
            adapter.UpdateAdapter(mFragments);
            adapter.notifyDataSetChanged();
        }else{
            adapter = new AdapterPagerFragment( mContext.getSupportFragmentManager(), mFragments); //需要在FragmentActivity类里
            this.setAdapter(adapter);
        }
    }
    //更新  控件某个item 数据（某个页面数据）
